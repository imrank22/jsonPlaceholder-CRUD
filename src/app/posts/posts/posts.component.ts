...
export class PostsComponent implements OnInit, OnDestroy {
  ...
  constructor( private postService: CommonService, private _fb: FormBuilder, private _snackbar: MatSnackBar, private route: ActivatedRoute ) { }
  ...
  fetchPosts(){
    ...
    let subscription = this.postService.getListOfPosts$().subscribe((data: Post[])=>{
      this.posts = data.splice(0, 4)
      this.posts.map(post =>{
        
        //fetching number of comments for each post
        let subscription1 = this.postService.getCommentsForPost$(post.id).subscribe(res=>{
  ...
  handleSearch(){
    let subscription = this.postService.getListOfPosts$().subscribe((data: Post[])=> this.posts = data.splice(0,4).filter(post => post.title.includes(this.search.toLowerCase())));
    this.subscriptions$.push(subscription)
  }
